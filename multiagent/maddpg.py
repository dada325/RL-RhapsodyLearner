...
from multiagent.agent import ActorCriticAgent

class MADDPG:
    def __init__(self, actor_dims, critic_dims, n_agents, n_actions,
                 min_actions, max_actions, alpha=1e-4, beta=1e-3, 
                 fc1=64, fc2=64, gamma=0.95, tau=0.01,
                 chkpt_dir='tmp/maddpg/', scenario='co-op_navigation'):
        self.agents = []
        chkpt_dir += scenario
        for agent_idx in range(n_agents):
            self.agents.append(ActorCriticAgent(actor_dims[agent_idx], critic_dims,
                               n_actions[agent_idx], n_agents, agent_idx,
                               alpha=alpha, beta=beta, tau=tau, fc1=fc1,
                               fc2=fc2, chkpt_dir=chkpt_dir,
                               gamma=gamma, min_action=min_actions[agent_idx],
                               max_action=max_actions[agent_idx]))
