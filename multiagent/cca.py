...
from memorys.memory import MemoryBuffer

class CollaborativeActorCriticAgent:
    ...
    def learn(self, memory, agent_list):
        ...
        actor_states = [T.tensor(actor_states[idx], device=device, dtype=T.float) for idx in range(self.n_agents)]
        actor_new_states = [T.tensor(actor_new_states[idx], device=device, dtype=T.float) for idx in range(self.n_agents)]
        actions = [T.tensor(actions[idx], device=device, dtype=T.float) for idx in range(self.n_agents)]
        ...
        old_actions = T.cat([actions[idx] for idx in range(self.n_agents)], dim=1)
        ...
        # Incorporate policies of other agents
        new_policies = [agent.actor(actor_states[idx]) for idx, agent in enumerate(agent_list)]
        all_policies = T.cat(new_policies, dim=1)
        
        # Actor loss based on the current policy and policies of other agents
        actor_loss = -self.critic.forward(states, all_policies).mean()
