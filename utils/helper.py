...
def calc_adv_and_returns(values, values_, rewards, dones, gamma=0.99, gae_lambda=0.95):
    device = 'cuda:0' if T.cuda.is_available() else 'cpu'
    deltas = rewards + gamma * values_ - values
    deltas = deltas.cpu().numpy()
    adv = [0]
    for step in reversed(range(deltas.shape[0])):
        advantage = deltas[step] + gamma * gae_lambda * adv[-1] * dones[step]
        adv.append(advantage)
    adv.reverse()
    adv = adv[:-1]
    adv = T.tensor(adv, dtype=T.float32, device=device, requires_grad=False).unsqueeze(2)
